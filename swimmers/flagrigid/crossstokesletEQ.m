function [G,M] = crossstokesletEQ(Gamma1,Gamma2,phi1,phi2,viscosity,alpha,tol)
%+========================================================================+
%|                                                                        |
...
%     M = cell2mat(M);
    ...
    Mass = integral(Gamma1,phi1,phi1); %%%%%%%%%%%%%%%%%%%%% WRONG HERE
    ONE = @(X,Y) ones(size(X,1),1);
    for n = 1:6
        % Indices
        i = ind{n}(1);
        ...
        name  = ['[ij/r+rirj/r^3]',num2str(i),num2str(j)];
        green = @(X,Y) 1/(8*pi*viscosity) .* femGreenKernel(X,Y,name,[]);
        G{n}  = integral(Gamma1,Gamma2,phi1,green,phi2);
        Ntensor{n} = integral(Gamma1,Gamma2,ntimes(phi1,i),ONE,ntimes(phi2,j));
        % Regularization
        G{n} = G{n} + 1/(8*pi*viscosity) .* regularize(Gamma1,Gamma2,phi1,name,phi2);
        if (i == j)
            M{n} = Mass;
        else
    ...
    M = cell(6,1);
    ind    = { [1 1] ; [2 2] ; [3 3] ; [1 2] ; [1 3] ; [2 3] };
    Mass = integral(Gamma1,phi1,phi1);
    for n = 1:6
        % Indices
        i = ind{n}(1);
        j = ind{n}(2);
        ...
        name  = ['[ij/r+rirj/r^3]',num2str(i),num2str(j)];
        green = @(X,Y) 1/(8*pi*viscosity) .* femGreenKernel(X,Y,name,[]);
        G{n}  = integral(Gamma1,Gamma2,phi1,green,phi2,tol);
